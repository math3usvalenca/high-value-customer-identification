...
def show_grouped_bars(
    ...
    if orient == "horizontal":
        sns.countplot(y=x_value, hue=hue_value, ax=ax, data=dataset, palette=palette)
        ax.set_title(title, size=12, color="dimgrey")
        # n_classes = len(dataset[x_value].unique().tolist())
        # print(ax.containers[0].get_height())

        all_heights = [[p.get_width() for p in bars] for bars in ax.containers]
        somas = []
        soma = 0
        for i in range(len(all_heights[0])):
            for x in all_heights:
                soma = soma + x[i]
            somas.append(soma)
            soma = 0
...
def donut_plot(
    ...
    colors = sns.color_palette("Set1", len(label_names))
...
def summary_analysis(num_df:pd.DataFrame) -> pd.DataFrame:
    
    """
        num_df: df with only numeric features
        return:DataFrame
    """
    ...
    df1 = pd.DataFrame(num_df.apply(np.std)).T
    df2 = pd.DataFrame(num_df.apply(min)).T
    df3 = pd.DataFrame(num_df.apply(max)).T
    df4 = pd.DataFrame(num_df.apply(lambda x: x.max()-x.min())).T
    df5 = pd.DataFrame(num_df.apply(lambda x: x.skew())).T
    df6 = pd.DataFrame(num_df.apply(lambda x: x.kurtosis())).T
...
def show_correlation_matrix(df:pd.DataFrame, method='pearson', figsize=(12,10)):
    ...
    plt.title(f'Correlation Matrix - {method}', fontsize=14, color='dimgrey')
    plt.show()